...
// Slug = {series}{ordinal}, e.g. "hasu6th". No venue (not reliably romanizable).
// Falls back to {series}{year} when a tour has no "Nth" ordinal.
function slugFor(tourName: string, seriesIds: string[], dateStart: string | undefined): string {
  const short = SERIES_SHORT[seriesIds[0]] ?? 's' + (seriesIds[0] ?? '');
  const ord = (tourName.match(/(\d+)(st|nd|rd|th)/i)?.[0] ?? '').toLowerCase();
  const suffix = ord || (dateStart ?? '').slice(0, 4);
  return (short + suffix).replace(/[^a-z0-9]/g, '') || short;
}
...
for (const [concertId, days] of legs) {
  ...
  eventRows.push([
    ...
    q(slugFor(first.tourName, seriesIds, dates[0])),
  ]);
